...
Task Compile {   
    ...
    $productionCodeProjects | ForEach-Object -Process {
        Invoke-MSBuild -Configuration "Release" -ProjectPath $_.FullName -OutputDirectory (Join-Path -Path $DirectoryStructure.ProductionCodeArtifactsDirectoryInfo -ChildPath $_.Directory.Name)
    }
    ...
    $testCodeProjects | ForEach-Object -Process {
        Invoke-MSBuild -Configuration "Release" -ProjectPath $_.FullName -OutputDirectory (Join-Path -Path $DirectoryStructure.TestArtifactsDirectoryInfo -ChildPath $_.Directory.Name)
    } 
}
